import { renderCatalogue, filterResources, escapeHtml } from './catalog.mjs';
const $ = s => document.querySelector(s);
const labels = {all:'All resources',articles:'Research articles',papers:'Papers',projects:'Open-source projects',datasets:'Datasets',benchmarks:'Benchmarks & environments',demos:'Video demos',about:'Collection policy'};
...
function readHash() {
  ...
  Object.assign(state, defaults, {
    ...
    sort:['newest','name'].includes(p.get('sort')) ? p.get('sort') : 'featured',
...
function render() {
  ...
  $('#catalogue').hidden=about;$('#about').hidden=!about;
  ...
  $('#result-count').textContent=`${results.length} of ${total} ${state.candidates?'candidates · excluded from curated counts':'resources'}`;
...
function reset(){const view=state.view==='about'?'all':state.view;Object.assign(state,defaults,{view});syncUrl();render();}
async function init(){
  try{
    ...
    if(document.modelContext?.registerTool){
      ...
      try{await document.modelContext.registerTool({name:'filter_research_resources',title:'Filter research resources',description:'Search game and embodied research, datasets, projects, benchmarks, and video demos. Updates the visible catalogue and returns source links.',inputSchema:{type:'object',properties:{query:{type:'string',maxLength:300},domain:{type:'string',enum:['all','game','embodied']},kind:{type:'string',enum:Object.keys(labels).filter(k=>k!=='about')}},additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:true},execute(input){
        ...
        if(input.kind!==undefined&&(!Object.hasOwn(labels,input.kind)||input.kind==='about'))throw new Error('Invalid kind');
