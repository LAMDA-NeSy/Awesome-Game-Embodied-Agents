...
function renderInstitutions(r) {
  ...
  const remaining=institutions.slice(3);
  const overflow=remaining.length?`<span class="institution-overflow" title="${escapeHtml(remaining.map(item=>item.name).join(', '))}" aria-label="${remaining.length} more affiliations">+${remaining.length}</span>`:'';
  return `<span class="paper-title-institutions" role="group" aria-label="Affiliations: ${escapeHtml(names)}">${marks}${overflow}</span>`;
}
function renderRating(r) {
  if(r.status!=='selected')return '';
  const stars=Array.from({length:5},(_,index)=>{
    const value=index+1;
    return `<button class="rating-star" type="button" data-rating-value="${value}" aria-pressed="false" aria-label="Rate ${value} out of 5" title="${value} out of 5">★</button>`;
  }).join('');
  return `<div class="paper-rating" data-rating-id="${escapeHtml(r.id)}"><div class="community-rating"><span>Community rating</span><strong class="community-rating-value" aria-label="No community ratings yet">—</strong></div><div class="personal-rating"><span class="rating-label">Your rating</span><div class="rating-buttons" role="group" aria-label="Rate ${escapeHtml(r.title)}">${stars}</div></div><span class="rating-status visually-hidden" aria-live="polite">Not rated</span></div>`;
}
...
function renderPaper(r,lookup) {
  ...
  return `<article class="resource-card paper-card ${stats?'has-stats':''}" id="resource-${escapeHtml(r.id)}">
    ...
    <div class="resource-body paper-body"><h3><a href="${escapeHtml(safeUrl(r.links.paper))}" target="_blank" rel="noopener noreferrer">${escapeHtml(r.title+shortName)}</a>${renderInstitutions(r)}</h3>
...
${stats?`<aside class="paper-sidebar" aria-label="Resources and statistics for ${escapeHtml(r.name)}"><nav class="paper-stats">${stats}</nav>${sourceNote}${renderRating(r)}</aside>`:''}
  </article>`;
}
