...
for(const p of ['dist/assets/app.js','dist/assets/catalog.mjs','dist/assets/updates.mjs','dist/assets/style.css','dist/README.md','dist/CITATION.cff','dist/CONTRIBUTING.md','dist/docs/collection-policy.md','dist/docs/metadata.md','dist/docs/assets/future-vision.png','dist/docs/assets/readme-emblem.png','dist/selection-audit.json','dist/maintenance.json','dist/updates.json','dist/CHANGELOG.md'])await fs.access(p);
const vision=await fs.readFile('dist/docs/assets/future-vision.png');
assert.equal(vision.subarray(0,8).toString('hex'),'89504e470d0a1a0a','Future vision asset is not a PNG');
const emblem=await fs.readFile('dist/docs/assets/readme-emblem.png');
assert.equal(emblem.subarray(0,8).toString('hex'),'89504e470d0a1a0a','README emblem asset is not a PNG');
const generatedReadme=await fs.readFile('README.md','utf8');
for(const heading of ['## Welcome','## Getting started','## Collection at a glance','## Our vision','## 🤝 Contributing','## 📖 Citation'])assert(generatedReadme.includes(heading),`README missing ${heading}`);
assert(generatedReadme.length<12000,'README should remain a concise guide to the website');
assert(generatedReadme.includes('img.shields.io')&&generatedReadme.includes('docs/assets/readme-emblem.png'),'README missing visual navigation');
assert(generatedReadme.includes('docs/assets/future-vision.png'),'README missing future vision artwork');
